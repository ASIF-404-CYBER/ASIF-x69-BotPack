...
const config = require("../../X-NiL.json");
...
global.utils = require("./xnild.js");
global.loading = require("./xnilc.js");
...
var ryukoValue;
try {
  global.client.ryukoPath = join(global.client.mainPath, "../configs/X-NiL.json");
...
var configValue;
try {
  global.client.configPath = join(global.client.mainPath, "../../X-NiL.json");
  configValue = require(global.client.configPath);
  logger.loader(`deploying ${chalk.blueBright('xnil')} file`);
} catch (e) {
  return logger.loader(`cant read ${chalk.blueBright('xnil')} file`, "error");
}
try {
  for (const key in configValue) global.config[key] = configValue[key];
  logger.loader(`deployed ${chalk.blueBright('xnil')} file`);
} catch (e) {
  return logger.loader(`can't deploy ${chalk.blueBright('xnil')} file`, "error")
}
...
try {
  if (!global.config.BOTNAME) {
    logger.error(`please enter your bot name in ${chalk.blueBright('X-NiL.json')} file`);
    process.exit(0);
  }
  if (!global.config.PREFIX) {
    logger.error(`please enter your bot prefix in ${chalk.blueBright('X-NiL.json')} file`)
  }
  if (global.config.author != "ryuko") {
    logger.error(`detected : author was changed at ${chalk.blueBright('X-NiL.json')}`);
    process.exit(0);
  }
  if (packages.author != "xnil") {
    logger.error(`detected : author was changed at ${chalk.blueBright('package.json')}`);
    process.exit(0);
  }
  if (packages.name != "xnil") {
...
try {
  var appStateFile = resolve(join(global.client.mainPath, "../../xnilstate.json"));
  ...
  logger.loader(`deployed ${chalk.blueBright('xnilstate')} file`)
} catch (e) {
  return logger.error(`can't read ${chalk.blueBright('xnilstate')} file`)
}
  ...
  console.log(chalk[`${cnslEvent.logger.colors.cmdStarter}`](`${cnslEvent.logger.strings.cmdStarter}`));
        for (const command of listCommand) {
          try {
            ...
            const configures = require(`../../X-NiL.json`);
            ...
            if (envConfig) {
              ...
              var ryukoPath = require('../configs/X-NiL.json');
...
(async () => {
  try {
    ...
    logger(`deployed ${chalk.blueBright('database')} system`, "X-NiL");
    logger(`deploying ${chalk.blueBright('login')} system`, "X-NiL")
    const botData = {};
    botData.models = models;
    onBot(botData);
  } catch (error) { logger(`can't deploy ${chalk.blueBright('database')} system`, "X-NiL") }
})();