...
module.exports.run = async ({ event, api, args }) => {
  ...
  if (args.length === 0) {
    return api.sendMessage(
      'Invalid usage. Please use the command with a font type and input text. Example: !font 1 Hello',
      event.threadID,
      event.messageID
    );
  }
  ...
  if (args[0].toLowerCase() === 'list') {
    ...
    return api.sendMessage(
      `Available font types total font 5: ${availableFontTypes}`,
      event.threadID,
      event.messageID
    );
  }
  ...
  if (!fontMap) {
    const availableFontTypes = fontMaps.map(item => item.name).join(', ');
    const errorMessage = `Invalid font type '${fontType}'.Available font types total font 5 : ${availableFontTypes}`;
